...
<cfquery name="queryGetCategory" datasource="#REQUEST.dataSource#">
...
<cfquery name="querySearchFavorite" datasource="#REQUEST.dataSource#">
		SELECT category_id, category, active, orderID
		FROM portal_category
		WHERE active = 1 AND category = 'Favorites'
</cfquery>
...
<script>
	function changeMenuID(currentRow)
	{
		...
		document.getElementById('tdcurrentMenuID'+currentRow).style.backgroundColor = 'blue';
		document.getElementById('tdcurrentMenuID'+currentRow).style.color = 'white';
	}
...
<cfform name="searchKeyword">
    ...
    &nbsp;&nbsp; Keywords: <cfinput name="keywords" type="text" size="30"/> <cfinput name="submit" type="submit" value="Submit"> (Seperate keywords by ";")
					
</p>
</cfform>

<cfset variables.counter = 0>
<div style="border:1px solid gray; height:702px; width:700px">
	<div style="float:left; border:0px dotted gray; height:700px; width:<cfoutput>#leftMenuWidth#</cfoutput>%; overflow-y: scroll; overflow-x:hidden">
		<table style="border-collapse:collapse" width="100%" border="1">
				...
				<tr class="#row_number#" >	       
            		<td id="tdcurrentMenuID#variables.counter#" style="background-color:white; cursor:pointer" onclick="javascript:ColdFusion.navigate('phamPortalContent.cfm?categoryID=#queryGetCategory.category_id#', 'photo_frame' );changeMenuID('#variables.counter#')">
						
                        <cfset theList = '3,60,65'>
            			<cfif  ListFind(theList, queryGetCategory.category_id, ',')>
							<span style="font-weight::bold; text-decoration:underline">[#queryGetCategory.category#]</span>
                        <cfelse>
                        	#queryGetCategory.category# 
						</cfif>
                        <cfif querySearchFavorite.category_id EQ queryGetCategory.category_id>
                       		<cfset variables.defaultHighlightRow = currentRow>
						</cfif>
					</td>   
				</tr>
				</cfoutput>
    ...
    <div style="float:right; border:0px dotted gray; height:auto; width:<cfoutput>#rightMenuWidth#</cfoutput>%;">
			<cfdiv id="photo_frame" name="photo_frame" bind="url:phamPortalContent.cfm?categoryID=3"/>
    </div>

</div><br />
<!---Highlight Favorites initially--->
<script>
    <cfoutput> 
        var #toScript(variables.defaultHighlightRow, "jsVar")#; 
    </cfoutput>
	changeMenuID(jsVar)
</script>
<div>
	<cfquery name="getDocuments" datasource="#REQUEST.dataSource#">
		SELECT * FROM document	
	</cfquery>

	<cfquery name="getYouTube" datasource="#REQUEST.dataSource#">
		SELECT * FROM youtube	ORDER BY dateLink DESC
	</cfquery>
		
	<cfif cgi.SERVER_NAME EQ 'localhost'>
		<cfset variables.uploadDocDir = 'C:\ColdFusion8\wwwroot\RCU\httpdocs\pham\documents\'>
	<cfelse>
	...
	<cfif isDefined('url.deleteThisDocument')>
			...
			<cfquery name="deleteDocument" datasource="#REQUEST.dataSource#" >
	...
	<cfif isDefined('url.deleteThisLink')>
			<cfquery name="deleteLink" datasource="#REQUEST.dataSource#" >
				DELETE FROM youtube							
			...
			<table class="uploadDocument" border="1">
				...
				<cfoutput query="getDocuments">
					<tr>
						...
						<td>
							<a href="#cgi.SCRIPT_NAME#?deleteThisDocument=#id#&docName=#docName#"  onClick="return confirm('Are you sure you wish to delete this record?');">	
								Delete
							</a>
						</td>
					</tr>
